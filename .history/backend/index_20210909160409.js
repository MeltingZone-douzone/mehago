...
io.of('/').adapter.subClient.on('message', (roomname, message) => {
    ...
    switch (msgToJson.validation) {
        ...
        case "update": io.to(roomname).emit('message:update:readCount', msgToJson);
            break;
        case "user left": io.to(roomname).emit('user left', msgToJson)
    }
...
io.on("connection", (socket) => {
    ...
    socket.on('leave', async (data) => { // data는 의미없음 지금
        ...
        const leaveMsg = {
            "validation": "user left",
            "message": `notice:${socket.id}님이 방을 나가셨습니다.`,
            chatMember
        }
        io.of('/').adapter.pubClient.publish(currentRoomName, JSON.stringify(leaveMsg));
