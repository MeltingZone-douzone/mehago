...
export default function Chatting2({ socket, participantObject, roomObject, chatRoomNo, searchMessage, setCurrentParticipants }) {
    ...
    useEffect(() => {
        ...
        socket.on('message:update:readCount', (msgToJson) => {
            ...
            console.log('msgToJson.chatMember: ', msgToJson.chatMember);
            const toIntArray = msgToJson.chatMember.split(',').map(Number);
            console.log(toIntArray);
            setCurrentParticipants(toIntArray);
        });
